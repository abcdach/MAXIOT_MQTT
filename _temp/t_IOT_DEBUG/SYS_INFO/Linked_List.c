...
{
    node* new_node = create(data,D0,D1,D2,D3,head);
    head = new_node;
    return head;
}
//#########################################################
node* append(node* head, int data,int D0,int D1,int D2,int D3)
{
    ...
    /* create a new node */
    node* new_node =  create(data,D0,D1,D2,D3,NULL);
...
node* insert_after(node *head, int data,int D0,int D1,int D2,int D3, node* prev)
{
    ...
    node *cursor = head;
    while(cursor != prev) cursor = cursor->next;
    if(cursor != NULL){
        node* new_node = create(data,D0,D1,D2,D3,cursor->next);
        cursor->next = new_node;
        return head;
    } else {
        return NULL;
...
node* insert_before(node *head, int data,int D0,int D1,int D2,int D3, node* nxt)
{
    ...
    if(head == nxt){
        head = prepend(head,data,D0,D1,D2,D3);
        return head;
    }
    /* find the prev node, starting from the first node*/
    node *cursor = head;
...
node* remove_any(node* head,node* nd)
{
    ...
    if(nd == head) return remove_front(head);
